...
#	print(args[2]) # assembly metrics (sequencing QC)
	...
	print(args[3]) # PopPunk GPSC file
	print(args[4]) # PopPunk clusters file
	print(args[5]) # Output file
	
	cmd_df <- plyr::join_all(list(gpsc,clusters,wgs), by='SampleID', type='full')
	#nrow(cmd_df)
	head(cmd_df);tail(cmd_df);nrow(cmd_df)

	# write results to xlsx file
	openxlsx::write.xlsx(cmd_df, paste(dir, args[5], sep = "/"), overwrite = T)
