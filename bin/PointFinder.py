...
def check_path(args_input, input_type, error_type = "Input Error", no_return = False):
    ...
    for infile in args_lst:
        ...
        if os.path.isdir(infile):
            size = os.listdir(infile)
        else:
            size = os.stat(infile).st_size
        if not size: print("Warning: %s, %s is empty"%(input_type, infile))
    if no_return:
        return None
    return args_input
...
parser = argparse.ArgumentParser(description="This program predicts resistance associated with chromosomal mutations based on WGS data.", prog="PointFinder.py")
...
parser.add_argument("-s", "--species", help="Species for point mutation detection.", required=True)
...
parser.add_argument("-m_p", "--method_path", help="Path to KMA or BLASTn program depending on the chosen method.", required=True)
